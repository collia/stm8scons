...
void term_print_int(uint16_t val,  uint8_t column, uint8_t row) {
    #define INT_STR_MAX_LEN 8
    char buffer[INT_STR_MAX_LEN+1];
    uint8_t i;
    buffer[9] = 0;
    for(i = 0; i < INT_STR_MAX_LEN; i++) {
        buffer[INT_STR_MAX_LEN-1-i] = val % 10 + '0';
        val /= 10;
        if(!val) break;
    }
    term_print(&buffer[INT_STR_MAX_LEN-1-i], column, row);
}
